...
def set_pion_in_matrice(
    matrice, coup, joueur
):  # (grid: State, player: Player, action: Action) -> State :
...
def afficher_matrice(matrice):
    ...
    max_element_length = max(
        len(str(matrice[i][j])) for i in range(n_rows) for j in range(n_cols)
    )

    # Afficher les colonnes avec les coordonnées
    print(
        " " * (max_element_length + 3), end=""
    )  # Espaces pour les coordonnées de ligne
    ...
    for i in range(n_rows):
        print(
            f"{-(i-sizeGrid+1):>{max_element_length}} |", end=" "
        )  # Coordonnées de ligne
        for j in range(n_cols):
            valeur = matrice[i][j]
            couleur = couleurs.get(
                valeur, ""
            )  # Obtenir la couleur ou utiliser la couleur par défaut
